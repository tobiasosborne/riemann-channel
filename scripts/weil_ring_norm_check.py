...
Claims under test (HANDOFF.md, section "Discussion 2026-09-14"):
  ...
  C4  Genus >= 2 "needs cancellations in the (-,-) block and presumably fermionic couplings".
      C4a checks the Hilbert-Schmidt chain sum_{odd eigenvalues} |mu|^2 <= ||E_odd||_HS^2
      <= 2 (sum_s ||a_s||_HS^2)(sum_s ||B_s||_HS^2) on random instances.  CORRECTION
      (prover, 2026-09-14, notes/ring-norm-tensor/astra-proofs.md T3.1): the last factor is
      NOT 2 Tr(E_++) Tr(E_--), because Tr(sum_s A_s (x) conj A_s) = sum_s |Tr A_s|^2 is not
      sum_s ||A_s||_HS^2; the printed labels "t+" and "t-" below are the Hilbert-Schmidt sums,
      and the genus bound g <= 1 is proved in T3.3 by the all-power inequality
      |Tr M^n|^2 <= Tr(E_++^n) Tr(E_--^n) instead.  Then a numerical search: bosonic-only
      C^{1|2} (fails), bosonic-only C^{2|2} (cancellation allowed; not found), and C^{1|2}
      with two fermionic species (found).
...
All counts are brute force in F_{p^n}.  Deterministic (seeded); the optimiser timings vary.
Run from the repo root: python3 scripts/weil_ring_norm_check.py > outputs/weil_ring_norm_check.txt
"""
...
banner('C4a  Bosonic block-diagonal C^(m|k): sum_odd |mu|^2 <= ||odd||_HS^2 <= 2 (sum||a_s||^2)(sum||B_s||^2)  (Schur + Cauchy-Schwarz), random instances')
for (m, k, d) in [(1, 1, 3), (1, 2, 4), (2, 2, 3), (1, 3, 5), (3, 2, 4)]:
    for trial in range(20):
        ...
        tpp = sum(np.linalg.norm(A[:m, :m]) ** 2 for A in As)   # sum_s ||a_s||_HS^2 (NOT Tr E_++, see docstring)
        tmm = sum(np.linalg.norm(A[m:, m:]) ** 2 for A in As)   # sum_s ||B_s||_HS^2 (NOT Tr E_--)
...
print('  NOTE: t+ t- are Hilbert-Schmidt sums of the tensors, not traces of the doubled blocks; the genus bound')
print('  g <= 1 follows from the all-power inequality |Tr M^n|^2 <= Tr(E_++^n) Tr(E_--^n) (astra-proofs T3.2-T3.3), not from here.')
...
check(resA.cost > 1e-6, '  bosonic-only C^(1|2) does not reach the count in this search (consistent with the no-go, not a proof)')
