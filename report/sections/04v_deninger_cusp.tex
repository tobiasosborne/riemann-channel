...
\claimstatus{prop:ihara-graph-of-surfaces}{proved}
...
\claimstatus{thm:eisenstein-not-radical-not-exit}{proved}
...
\claimstatus{thm:prime-level-scattering}{proved}
...
\claimstatus{thm:twisted-theta-edges}{proved}
...
is inner and pure Blaschke unconditionally: zeros $w_\zero = \gam{}/2+i(1-\sigs)/2$ in $\mathbb C_+$ with multiplicity, no
...
\claimstatus{thm:level-model-decomposition}{proved}
...
disc shift is a cogenerator convention. Modes: $\mathcal M^{-1}K_{\Theta_\chi} = \overline{\mathrm{span}}\{(\log y)^jy^{-(1-\sigs)/2-i\gam{}/2}1_{y>1}\}$
...
\claimstatus{prop:level-positivity-criterion}{proved-conditional}
On a finite, functional-equation-stable selection of arithmetic zero modes with complete chains and partners, with
$D = A+\tfrac14$ and eigenvalues $d_\zero = (\sigs-\tfrac12)/2-i\gam{}/2$, a positive metric making $e^{\tsg D}$ unitary for all real
...
\claimstatus{obs:h-arith-q-verdict}{refuted}
