...
\claimstatus{prop:class-group-bond}{sketched}
...
the theta divisor $\{h^0>0\}\subset\mathrm{Pic}^{g-1}$ (whose fixed locus is the $2$-torsion coset, in general
larger). The two poles of $Z$ are the two constant terms of $\Theta$, $a = 0$ and its Fourier image.
\end{proposition}

\begin{proposition}[The cusps of the tree quotient are the class-group bond]
\label{prop:cusps-are-class-group-bond}
\claimstatus{prop:cusps-are-class-group-bond}{sketched}
Let $R = O(C\setminus P_0)$, $\deg P_0 = 1$. (a) The cusps of $\mathrm{GL}_2(R)\backslash\mathcal T$ are in bijection with
$\mathrm{Pic}(R)\cong\mathrm{Pic}^0(K)$ (Serre; \cref{cit:lorscheid-cusp-count}): one for D2, four for D3. The
hedgehog's spikes are the units-invariant GL$_1$ bond $\ltwo{\mathrm{Pic}^0}$, and H-CLASS (\cref{obs:h-class})
says the GL$_2$ scattering matrix at equal depth is the Fourier transform on it. (b) For a class character $\chi$
...
$\sum_{\mathrm{Pic}^n}\chi = 0$): the three monomial channels of \cref{thm:d3-channels} are the three nontrivial
characters of $\mathbb Z/4$, and the zeta channel is $\chi = 1$; this is \cref{prop:character-channel-graded-bond}
at the level of $\mathrm{Pic}^0$. (c) At genus one $\dim\ltwo{\mathrm{Pic}^0} = h = P(1)$ while $\dim K_{\mathrm{HW}} = 4$:
they coincide for D3 ($h = 4$) and not for D2 ($h = 1$), so ``one cusp per class'' is not ``one exit per zero mode''
...
the odd part $\jtheta-1-y^{-1/2}$ of the bond state, and the constant terms $1$, $y^{-1/2}$ (the even sector, the
poles $s = 1, 0$) are exchanged by the automorphism $y\mapsto1/y$. Riemann's 1859 argument is the $\mathbb Q$ case of
...
lane's counterexample $\qs = 2$, $h = 6$ has roots $-1,-2$). Any metric for which $\Frob$ is normal is diagonal in its
eigenbasis, and reality and the bipartite sign fix the ratios, so ``the symmetric ray of \cref{thm:arithmetic-metric}
