...
\claimstatus{prop:theta-functional-bond-state}{proved}
Let $f = 1_{O_{\mathbb A}}$ and $x$ an idele with divisor $D$. (a) $\Theta(xf) = \sum_{a\in K}1_{O_{\mathbb A}}(xa) = \qs^{h^0(D)}$.
(b) With $\mathrm{vol}(\prod_vO_v^\times) = 1$ and $\re s>1$ (elsewhere by continuation), $\int_{\mathbb A^\times/K^\times}(\Theta(xf)-1)|x|^s\,d^\times x =
...
$\{2[D] = [K_C]\}$, a $\mathrm{Pic}^0[2]$-torsor when nonempty, of order at most $2^{2g}$). The two poles of $Z$ are the two constant terms of $\Theta$, $a = 0$ and its Fourier image.
\end{proposition}

\begin{proposition}[The cusps of the tree quotient are the class-group bond]
\label{prop:cusps-are-class-group-bond}
\claimstatus{prop:cusps-are-class-group-bond}{proved-conditional}
...
\claimstatus{prop:riemann-theta-dilation-bond}{proved}
...
\label{conj:h-theta}
\claimstatus{conj:h-theta}{refuted}
...
Refuted 2026-09-21 (\cref{thm:theta-vector-cyclic}): the theta vector's forward dilation orbit is the whole bond and its outgoing half is outer, so no cyclic-and-cut construction gives $\KS$; what survives is \cref{thm:symbol-edge-quotient} and \cref{thm:gl1-bad-zero-defect}.
\end{conjecture}

\begin{observation}[What the bond reading does not supply: the metric]
\label{obs:gl1-bond-no-metric}
\claimstatus{obs:gl1-bond-no-metric}{proved}
