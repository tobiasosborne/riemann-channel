...
With the boundary measure $d\tauM/2\pi$ fixed here (\cref{sec:rebound-state} uses $d\tauM$, whence its constant $-i\sqrt{2\pi}$), the kernel $k_{w_\zero}$ is $-i\,y^{-(1-\sigs)/2 - i\gam{}/2}1_{y>1}$ in the $y$-picture, and, unconditionally,
\[
\widehat{\phantom{x}}^{-1}\KS = \overline{\mathrm{span}}\{(\log y)^j\,y^{-(1-\sigs)/2-i\gam{}/2}1_{y>1}:\ \cxi(\zero) = 0,\ 0\le j<m_\zero\},
\]
...
$y^{-i\gam{}/2}$ moves under $U_\tsg$ by $e^{+i\tsg\gam{}/2}$, the mode under the adjoint compression by
$e^{-i\tsg\gam{}/2-\tsg(1-\sigs)/2}$. Under \cref{asm:rh} every mode is ``cut the absorption character to the outgoing
...
\claimstatus{conj:h-theta-1}{open}
