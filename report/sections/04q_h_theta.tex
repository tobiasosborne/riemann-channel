...
compressed semigroup $\Zt$ and its adjoint $C_\tsg$ as in \cref{sec:rebound-state}; a zero $\zero = \sigs+i\gamma$
gives the zero $w_\zero = \gamma/2 + i(1-\sigs)/2$ of $\Theta$ and the Cauchy kernel $k_w(\tauM) = (\tauM-\bar w)^{-1}$.
Write $\cxi_{\mathrm{crit}}(\tauM) := \cxi(\tfrac12 + 2i\tauM)$, even, real on $\mathbb R$, entire, with zeros
$a_\zero = \gamma/2 - i(\sigs-\tfrac12)/2$, all real iff RH.
...
\qquad v_{\zero,\pm} = \pm\gamma/2 + i\sigs/2,
\]
...
bad-zero channel}: one Jordan chain of length $m_\zero$ per zero with $\sigs>\tfrac12$, at $w^{(1)}_\zero = \gamma/2 +
...
real, even, positive-definite function whose spectral density $|\widehat{\gth}|^2$ vanishes exactly at $u = \gamma/2$
