...
\begin{proposition}[Transfer matrix for the quadratic family: the two forms]
\label{prop:as-transfer-matrix}
\claimstatus{prop:as-transfer-matrix}{proved-conditional}
For $g(x) = \sum_{j=0}^{\asrange}a_jx^{1+\qs^{j}}$ and every $n\ge1$, including
$n\le\asrange$: $\Tr\Tmat_g^{\,n} = \sum_{x\in\Fq^n}\addch(\Qring{g}(x))$ with $\Qring{g}$
the ring form of matrix $\Ppoly{g}(\cshift)$, and $\expsum{n}(g) =
\sum_{x}\addch(\Qtrace{g}(x))$ with $\Qtrace{g}$ the trace form of matrix
$C\,\Ppoly{g}(\cshift)$ in normal-basis coordinates (\cref{def:as-ring-forms}). The two
forms have the same radical $\radn{n}$, so $|\expsum{n}(g)| = |\Tr\Tmat_g^n|$ and
$\log_{\qs}(|\expsum{n}|^2/\qs^n) = \dfix{n}$ is a nonnegative integer.
\end{proposition}

Two remarks on the statement, both corrected on 2026-09-12 night. First, the founding
session's reading of the even-$n$ sign flip as the normalisation $\alphaw{i} =
-\lambda_i(\Tmat)$, ``uniformly in $n$'', is false in general: it holds exactly when
$\Ppoly{g}(-1)\ne0$, and the exact law $\expsum{n} = (-1)^{n-1}\dsign{n}\Tr\Tmat_g^n$ with
the radical sign $\dsign{n}$ is \cref{thm:as-sign-law} of \cref{sec:artin-schreier-super},
where the corrected Frobenius block is built. Second, a self-dual normal basis exists only
for odd $n$; in a general normal basis the trace form is $C\,\Ppoly{g}(\cshift)$, not
$\Ppoly{g}(\cshift)$, and the discrepancy between the two Gauss sums is precisely the
discriminant of $C$ on the complement of the radical, which is Stickelberger's sign
$(-1)^{n-1}$ corrected on the radical.
...
\claimstatus{prop:as-unitarity}{proved}
If $a_{\asrange}\ne 0$ then $\Tmat\Tmat^{\dagger} = \qs\,I$ on the
$\qs^{\asrange}$-dimensional bond (the scalar quadratic Gauss sum when $\asrange = 0$).
Hence $\Tmat/\sqrt{\qs}$ is unitary, every eigenvalue of $\Tmat$ is nonzero, semisimple
and of modulus $\sqrt{\qs}$, and, through the corrected Frobenius block of
\cref{thm:as-frobenius-block}, the Weil bound $|\alphaw{i}| = \sqrt{\qs}$ holds for the
...
which scaled unitarity makes trivial. The proof with the output-row index bookkeeping of
\eqref{eq:as-transfer-entries} made explicit is the prover's T7.6
(\cref{sec:artin-schreier-super}); the even-$n$ sign is settled there as well.
...
argument of \cref{prop:as-unitarity} is now proved, and the transfer unitary is
identified as a Weil-representation operator in \cref{sec:artin-schreier-super}.
