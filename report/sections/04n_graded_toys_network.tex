...
\claimstatus{thm:diagram-linearisation}{proved}
...
0\oplus\ker(I-C)$, and, when no $c = 1$ junction carries a loop and the Schur bracket of
\cref{prop:multi-exit-spectrum}(iii) is invertible, $M$ has $\dim\ker(I-C)$ Jordan blocks of size two at $0$: the
delay sector seen on the core (a loop at a $c = 1$ junction gives $\operatorname{ord}_0p = 1$ and a block of size one). (d) For $C = 0$ and
$T_X = \Aadj_X/\sqrt{\qs}$ with $\Aadj_X$ the adjacency matrix of a $(\qs+1)$-regular graph, $\det(1-zM_0) = \det(I-\uz\Aadj_X+\qs\uz^2)$, $\uz = z/\sqrt{\qs}$, so by Bass's theorem
...
non-backtracking operator plus a correction of rank the number of distinct junction vertices, the bound sheet
being $\det(1-zM)$ and the outgoing sheet $\det(z-M)$. The cores of D2 and D3 are not regular by themselves (the
cusp ray carries the missing unit of degree at the junction), so for them only the determinant identity transfers.
\end{theorem}
...
\claimstatus{thm:scattering-superdeterminant}{proved}
...
with eigenvalue $\qs$ (on $|z| = 1$) cancel. Under $z^2 = e/\qs$ the eigenvalues with $|e|<\qs$, whose images lie in the disc,
...
\claimstatus{prop:cavity-superdeterminant}{proved-conditional}
...
parity of a line is its cohomological degree mod $2$ on the untwisted copy and the degree plus one on the
Tate-twisted copy; flipping the untwisted copy instead inverts the ratio, so the operator $E_S$ involves the choice
of which copy is flipped, but the graded divisor, read off the rational function, does not: the pole is even and the
zeros are odd. The prefactor $z^{-2}$ is the delay sector, a cut artefact (\cref{prop:multi-exit-spectrum}(iii)).
\end{proposition}
...
\claimstatus{prop:genus-prefactor-forced}{proved}
Let a one-cusp diagram with one junction of coupling $c = 1$, no loop at the junction and invertible Schur
bracket (so $\operatorname{ord}_0p = 2$), cut at the junction, have the Perron pair as its only visible bound states,
the $4g$ numbers $z^2\in\{\alphaw{i}/\qs\}$ as its nonzero resonances, all simple, and every other root of $p$ on the
unit circle, with monic unimodular factor $D$ and $s_D = D(0) = (-1)^{m_{+1}}$
...
$\operatorname{ord}_0p = 2\dim\ker(I-C) = 2$ for every genus. For D2 ($g = 1$, $s_D = 1$) this is
\cref{thm:d2-zeta}; D3 has four cusps and lies outside the hypothesis (its even channel has $s_D = -1$ and the sign
of \cref{thm:d3-channels} comes from $(-1)^h$ in \cref{thm:multi-exit-scattering}); at $h = 1$ the formula $|[z^m]p| = \qs^{1-g-(h-1)(g-1)}$ of
...
\claimstatus{thm:renewal-graded-mps}{proved}
...
the odd half of the point count of $y^2+y = x^3+x+1$ over $\Fqn{k}$. (d) The retained graded divisor of $\Edb$ (nonzero
eigenvalues) is $\netmult(1) = 2$ (the vacuum and $\rdens_\infty$), $\netmult(\lambda) = -2m_\lambda$ at every nonzero
$\lambda\in\operatorname{spec}Z$ (the odd sector carries $\operatorname{spec}Z$ and its conjugate, and the
resonance set is conjugation-closed), and $\netmult = m_0$ at the remaining nonzero eigenvalues of
$\mathcal E_{\reb}$; the ring zeta is
$1/\sdet(1-\uz\Edb) = \prod_n(1-\uz z_n)^{m_n}(1-\uz\bar z_n)^{m_n}/((1-\uz)\det(1-\uz\mathcal E_{\reb}))$, the product
over modes: its zeros are the resonances and their conjugates. (e) The nonzero resonances share one modulus $r$
(RH on the retained divisor, i.e.\ on $K_{\mathrm{HW}}$ for D2) iff every nonzero odd eigenvalue of $\Edb$ has
modulus $r$; the even sector has two fixed points (\cref{prop:graded-stationary-segment}), so
...
\texttt{scripts/graded\_toys.py} ($512$ assertions, $12$ s, seed $20260920$; \texttt{outputs/graded\_toys.txt};
ledger D1--D51 in \texttt{notes/graded-toys/numerics.md}; $498$ pass, every one of the $14$ failures a wording or scope
correction of the brief, each paired with a passing corrected assertion). This shard's part: the linearisation on D2, D3
and random cores with one to three cusps, the Jordan structure and its loop counterexample, the Bass identity on five
regular graphs (with the trivial factor $\pm\qs^{-1/2}$); the superdeterminant identity, the disc classification and the
functional equation exactly for both curves and a synthetic genus-two Weil polynomial; $1/R$ against
$z^{-2}\sdet(1-E_S/(\qs z^2))$ at eight points on D2 and D3; the prefactor and first nonzero coefficient ($[z^2]p_2 = -1$,
$[z^4]p_3 = -1$); the graded MPS of D2 with a modal and a random reset: normalisation, the transfer channel, the even and
odd sectors, the explicit ring vectors to $L = 4$, both closures, the point counts $\Ncount{k} = 1, 5, 13, 25, 41$ by
enumeration in $\Fqn{k}$ against $\Tr Z^{2k}$, the ring-zeta product formula to $10^{-14}$, the odd multiplicities
$-2$, the value $0$ with multiplicity $20$ resp.\ $18$ in the even sector against $4$ in the odd one ($\netmult(0) = 16$,
$14$, invisible to the ring zeta), the fixed-point segment.
\end{numerical}
