...
\claimstatus{prop:constant-mode-mixed-sheets}{proved}
...
\claimstatus{lem:funnel-first-order}{proved}
...
$d|z_i|/df = \re(\bar z_i\,dz_i/df)/|z_i|$. For a bipartite core ($\varepsilon T_X\varepsilon = -T_X$), since $P_v$ is real and diagonal and commutes with
$\varepsilon$, the roots of $p_f$ stay closed under $z\mapsto-z$ and $z\mapsto\bar z$ for every $f$, so the Hasse--Weil quartet remains one Klein orbit
...
\texttt{scripts/graded\_toys.py}, ledger D33--D37: the constant-mode identity of \cref{prop:constant-mode-mixed-sheets}
...
\claimstatus{lem:even-letters-two-fixed-points}{proved}
...
\claimstatus{thm:glued-graded-toy}{proved}
...
\claimstatus{prop:shared-exit-tradeoff}{proved}
...
$\langle a_m,a_n\rangle\reb$); if the value $z_n\bar z_m$ is simple in $\Ad(Z)$ it persists iff $\langle l_n|\reb|l_m\rangle = 0$
for the dual eigenvectors (\cref{prop:rebound-persistence-secular}), and when it is degenerate, as on a real core where
the conjugate modes carry the same value, the rank-one perturbation removes at most one copy, the drop being decided
by the sum of the residues over the degenerate eigen-operators, not by each separately. On a
...
\claimstatus{thm:frobenius-channel-expander}{proved-conditional}
...
arithmetic metric (\cref{thm:arithmetic-metric}), $r = \qs^{-1/4}$ and $U^2\simeq(\Frob|_{H^1}/\sqrt{\qs})\otimes1_2$
...
\claimstatus{prop:no-selfadjoint-one-mode-exits}{proved}
...
scattering matrix of any self-adjoint diagram; any constant-basis splitting of the quartet is into conjugation-closed
sets, hence at most into two pairs (whether a connected core attains the two-pair splitting is not established). On
$K_{\mathrm{HW}}$ the bipartite sign and complex conjugation generate a Klein four-group acting simply transitively on
the four modes; the symmetric arithmetic metric is, up to scale, the unique diagonal metric it fixes, and in it the four one-mode exits are permuted by the group.
\end{proposition}
...
\claimstatus{prop:character-channel-graded-bond}{proved-conditional}
...
$\sqrt{\qs}$ \cite{rosen2002function} (not byte-cited; the source is not in the repository). With $E_\chi = (\Frob_\chi)_-\oplus\Pi\Frob_\chi(-1)$ and $w = \qs^{-2s}$, the
...
\texttt{scripts/graded\_toys.py}, ledger D47--D51: all $25$ nontrivial characters mod $T^3-T-1$ (primitive, $\deg L = 2$,
...
imprimitive characters' modulus-one roots; the trivial channel at $g = 0$. Also this shard's glued toy and Frobenius channel (D38--D46:
...
core; G5(a,b) in full; conjugation-closed disc zero sets for real and complex exit
