...
the squarefree gas with $\operatorname{M\ddot ob}(n) = (-1)^{\#\text{primes}}$ its fermion parity; each finite factor
$1-p^{-s}$ vanishes on $\re s = 0$; this says nothing about the continued product $1/\zeta$, whose poles in
the strip are the zeros of $\zeta$ (\cref{obs:phase-side-verdict}).
\end{corollary}
...
uniform rate on every trivial--nontrivial coherence, leaves the trivial (pole) sector alone, and with inversion-symmetric rates produces no
frequency (\cref{prop:galois-dephasing-rates}); taken as a full average per step it is a character measurement and removes the
