...
e^{-i\tsg\bar\lambda}k_\lambda$. With $\lambda = \gam{n}/2 - i\sigs_n/2$ the eigenvalue is
$\exp(-\sigs_n\tsg/2 - i\gam{n}\tsg/2)$: one mode per zero, decay rate $\sigs_n/2$,
frequency $\gam{n}/2$. Hence the Riemann hypothesis holds if and only if every mode of
...
\claimstatus{prop:ringnorm-trace}{sketched}
The distributional trace of $\Zt$ is supported at the ring lengths $\tsg = 2\log n$ with
weight $\vM(n)\,n^{-1/2}$.
\end{proposition}

After the $e^{-\tsg/4} = n^{-1/2}$ decay of the modes this is the $\sigs=1$ L\'evy measure
$\jump{1}$ weighted by jump length, $\vM(n)/n$. In the MPS dictionary of
...
\claimstatus{obs:ringnorm-sign-correction}{sketched}
...
positive-time uncentred prime part is $-2\vM(n)/n$. \Cref{prop:ringnorm-trace} is right
about the support and off by the factor $-2$ in the weight; it is to be restated.
\end{observation}
