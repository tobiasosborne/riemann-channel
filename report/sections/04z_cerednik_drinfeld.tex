...
\claimstatus{thm:shimura-curve-quotient-dual-graph}{sketched}
...
\claimstatus{thm:monodromy-pairing-cycle-metric}{sketched}
...
\claimstatus{thm:whole-curve-zeta}{sketched}
...
\claimstatus{prop:toric-filtration-not-frobenius}{sketched}
...
\claimstatus{prop:degree-and-norm-ratio}{sketched}
...
\texttt{scripts/cerednik\_drinfeld.py} (\emph{partial}: $13$ assertions, all pass; the lane was cut by the spend limit; \texttt{outputs/cerednik\_drinfeld.txt}; ledger in \texttt{notes/cerednik-drinfeld/numerics.md}; written without reading the prover's scripts): the complex, the bipartite sign, $T_1D = DA_{17}$, the integral cycle basis $Z$, $M = Z^{t}Z\ge I$, and $\det M = \tau(Y) = 2^{217}3^{92}5^{33}\cdot7$ exactly (Sylvester, reduced Laplacian, spectral formula). Pending: the Smith normal form, $L^{t}M = ML$, $T_{17}-18$ on the component group, $G_M$.
\end{numerical}
