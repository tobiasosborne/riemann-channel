/* eigmin.c: certified minimal eigenpair of a real symmetric ball matrix, ball LDL^T inertia, and
 * the even-simple certificate.
 *
 * VERBATIM PORT of zst/src/eigmin.c (356 lines), with only the three public names changed from the
 * zst_ prefix to the ihz_ prefix; every static helper and every line of logic is unchanged.  The
 * file is generic: lanes/code-audit.md:14 (section 1, classification table) records it as
 * "generic, reusable as-is -- nothing in the body refers to zeta, the window, or the Loewner
 * structure", and plan.md:356-368 (section 4.3) makes it the ball stage of this library.
 *
 * What is certified, and its ground truth (unchanged from zst): refs/src/2511.22755/mc2arXiv.tex,
 * Definition `even-simple` l.850 (smallest eigenvalue simple, eigenvector even) and Lemma `key`
 * l.863-870 (T >= 0 with a one-dimensional kernel).  For a graph the Ihara reading of the same
 * hypothesis is lanes/theory.md:433-455 (IH-15) and 456-481 (IH-16, IH-17): half of "even-simple"
 * evaporates over Z, the surviving half is simplicity, and it FAILS at and above the critical
 * window (IH-18/IH-19) -- ihz_certify_even_simple requires eps > 0 strictly and returns 0 there.
 *
