...
static void dir_ab(arb_ptr a,arb_ptr b,slong N,const arb_t x,ulong X,slong prec)
{
    zst_weil_t W;fmpz_t D;fmpz_init(D);fmpz_set_si(D,discriminant);
...
static void rayleigh_table(double xd,ulong X,const arb_t L,const pair_t *p,slong prec)
{
    ...
    dir_ab(a,b,N,x,1,prec);rayleigh(A,a,b,v,N,p->par,prec);arb_set(S,A);
    ...
    dir_ab(ta,tb,N,x,X,prec);
    check(vec_overlaps(a,ta,N+1)&&vec_overlaps(b,tb,N+1),"character-weighted local terms reproduce generic builder");
...
{
    /* X=1 includes gamma + log(q) I, no poles, no prime atoms. */
    ...
    arb_mat_t E,O;arb_mat_init(E,N+1,N+1);arb_mat_init(O,N,N);dir_ab(a,b,N,x,1,cp);
...
static void mode_axisN(double xd,slong Nmax,slong prec,const char *cmp,slong cprec)
{
    ...
    flint_printf("# axisN D=%wd kappa=%d x=%g Nmax=%wd prec=%wd cprec=%wd\n",discriminant,discriminant<0,xd,Nmax,prec,cprec);
    dir_ab(a,b,M,x,X,prec);zst_even_block(E,a,b,M,prec);zst_odd_block(O,a,b,M,prec);
    ...
    for(slong n=1;n<=Nmax;n++)if(selected[n]) {
        ...
        fprintf(stderr,"D=%ld x=%g N=%ld eigenpair/control finished %.1fs\n",(long)discriminant,xd,(long)n,now()-start);fflush(stdout);
    }
    ...
    arb_t gamma;arb_init(gamma);check(reference_zero(gamma,discriminant,cprec),"Hardy Z interval Newton reference zero");
    flint_printf("REFERENCE D=%wd gamma1=%s certified_unique=1 index=PARI_or_sign_scan\n",discriminant,sb(gamma,30));
...
static void mode_axisx(slong N,double xmax,slong prec,int fixedL)
{
    ...
    flint_printf("# axisx D=%wd kappa=%d N=%wd xmax=%g prec=%wd fixedL=%d\n",discriminant,discriminant<0,N,xmax,prec,fixedL);
    flint_printf("# CCM changes window and primes; fixedL changes only cutoff, including zero character weights.\n");
    for(slong i=0;i<K;i++) {
        double xd=fixedL?xmax:ks[i],start=now();arb_set_d(x,xd);zst_window_L(lengths+i,x,prec);
        dir_ab(a,b,N+1,x,ks[i],prec);
        if(fixedL) {
        ...
        fprintf(stderr,"D=%ld knot x=%g X=%lu %.1fs\n",(long)discriminant,xd,ks[i],now()-start);fflush(stdout);
    }
    for(slong i=0;i<K;i++) {
    ...
    arb_t gamma;arb_init(gamma);check(reference_zero(gamma,discriminant,prec),"Hardy Z interval Newton reference zero");
    flint_printf("REFERENCE D=%wd gamma1=%s certified_unique=1 index=PARI_or_sign_scan\n",discriminant,sb(gamma,30));
    for(slong i=0;i<K;i++) {
...
int main(int argc,char **argv)
{
    ...
    for(int i=1;i+1<argc;i+=2) {
        if(!strcmp(argv[i],"--D"))discriminant=atol(argv[i+1]);
    ...
    flint_printf("# checks: %wd run, %wd failed\n",n_checks,n_fail);flint_cleanup();return n_fail?1:0;
}
