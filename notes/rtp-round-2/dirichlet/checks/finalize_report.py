...
**NUMERICAL / SHARPENED.** The Schur interval lemma is unchanged without poles; the Schur half-width does not follow eps; the comparison error tracks eps; and N saturation is a Weyl-count effect. The coefficient 1.7 and a determinant argmin as an eigenvalue-convergence criterion require qualification. The fixed-window partial forms can have odd minima, and their prime-free control need not be indefinite. Round 1's zeta numerical statements remain intact; no zeta run or shard was changed here.
...
for pp in files+[ROOT/'zst/tools/rtp2_dirichlet.c',ROOT/'zst/tools/rtp2_dirichlet_run.sh',ROOT/'zst/tests/data/dirichlet_ref.txt',ROOT/'zst/build/libzst.a']:
