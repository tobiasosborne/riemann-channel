...
**NUMERICAL — COMPARISON STEP at x=50.** At the final N=940, the first-only pipeline certifies the preceding positive range root-free in 2353 subdivision cells and gives `|z1-gamma1|=[1.627044261059612e-254 +/- 2.99e-270]`, ratio to eps `[10803.83972957315 +/- 2.95e-12]`. The prime-free control has negative inertia `(3,4)` in the even/odd blocks. Completion time: **41.511028 s (floating)**. The ratio to the quoted prolate expression is about **10.8184**; the certified slope from the final x=25 point is **5.40440468932** digits per unit x, versus **5.40332001615 (floating)** for the prolate expression. The first three converged endpoints therefore remain consistent with the prolate exponential; the larger points are still needed for the fit and verdict.

**NUMERICAL (continuation efficiency).** The driver also permits the preceding prime-side unit eigenvector, zero-padded, as an approximate start. The residual and gap checks still certify each new point independently. A check at x=13,N=360,400,440 gives identical printed epsilon balls and the same stopping decision, while reducing the later iteration counts from 16 to 12 (`checks/warm_check.txt`). The x=70 continuation uses this option and 24 threads after its initial completed segment, with the same 4600-bit PD certificate. Its input epsilon ball is carried solely for the convergence ratio. This reduces repeated work while extending the original maximum N if needed; the split and settings are explicit in the output and runner.
