...
- **What it does not settle.** A mechanism forcing the pairs to be degenerate. In positivity form (Weil, Connes): RH $\iff$ the zero-side function $F(u) = \sum_\rho e^{(\rho-1/2)u}$ is positive-definite on the dilation group $\iff$ its spectral measure $\sum_\rho\delta_{t_\rho}$ is a positive measure on the real line, which by Bochner is the same as all $t_\rho$ real. In operator words: write the transfer generator on the subleading sector as $T = -\tfrac12 + iH$; then $H$ has eigenvalues $\gamma_n - i(\beta_n - \tfrac12)$, and **RH says $H$ is Hermitian**, i.e. the transfer semigroup is a uniform damping times an oscillation with real frequencies. $H$ is the Hilbert–Pólya operator. It is *not* the entanglement Hamiltonian of the state: that object is built from the fixed points of $E$ alone, is always Hermitian, and does not see the subleading spectrum. The transfer matrix is to $H$ what the Hashimoto edge matrix is to the adjacency matrix of a graph: the non-Hermitian object carries the complex pairs, the Hermitian one carries their symmetric combination, here $\rho(1-\rho) = \beta(1-\beta) + \gamma^2 + i\gamma(1-2\beta)$, which is real iff $\beta = \tfrac12$. For graphs the Hermitian object is given and its reality is a theorem; for the primes it is the conjecture. Proving it means exhibiting that state, or a Stinespring form of $Z(t)$ with the prime dilations as Kraus operators, in a way that makes the positivity manifest. That is the open question the repo isolates, and the graph lesson says the Kraus structure alone will not do it; some arithmetic rigidity has to enter, playing the role weights play for curves.

## 8. Erratum on words

An earlier draft, and the founding conversation, used "entanglement Hamiltonian" for $-\log E$. That is wrong: the entanglement Hamiltonian of an MPS is $-\log\rho_A$ with $\rho_A$ built from the fixed points of $E$, and it is Hermitian by construction. The operator whose spectrum contains the zeros is the transfer generator, which is non-Hermitian in general; its eigenvalues' phases are the oscillation frequencies of the ring norms. Section 7 states the correct relation.
